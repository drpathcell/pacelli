...
/// Screen for creating a new manual entry.
class CreateManualEntryScreen extends ConsumerStatefulWidget {
  final String householdId;
  const CreateManualEntryScreen({super.key, required this.householdId});

  @override
  ConsumerState<CreateManualEntryScreen> createState() =>
      _CreateManualEntryScreenState();
}

class _CreateManualEntryScreenState
    extends ConsumerState<CreateManualEntryScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final entry = await repo.createManualEntry(
      ...
      ref.invalidate(manualEntriesProvider(widget.householdId));

      if (mounted) {
        // Navigate to the newly created entry.
        context.pop();
        context.push('${AppRoutes.manual}/${entry.id}');
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(context.l10n.manualCreateTitle),
        actions: [
          TextButton.icon(
